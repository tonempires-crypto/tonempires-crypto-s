    ...
    async function fetchMarketDynamics() {
      try {
        ...
        if (prices) {
          const dynamicData = [
            { 
              name: t('dash.resources.oil').toUpperCase(), 
            ...
            { 
              name: t('dash.resources.gold').toUpperCase(), 
            ...
            { 
              name: t('dash.resources.iron').toUpperCase(), 
            ...
            { 
              name: t('dash.resources.wheat').toUpperCase(), 
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        ...
        <span className="text-[10px] font-mono text-zinc-500 uppercase">{t('market.indexing')}</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-2">
        <div>
          <h2 className="text-xl font-black uppercase tracking-tight">{t('market.title')}</h2>
          <p className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">{t('market.subtitle')}</p>
        </div>
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <span className="text-[10px] font-mono text-zinc-500 uppercase">{t('market.vol')}: {item.volume}</span>
              </div>
            </div>
            
            <div className="text-right">
              <div className="text-lg font-mono font-bold">{item.price} <span className="text-[10px] text-zinc-500">TON</span></div>
              <div className="text-[8px] font-mono text-zinc-600 uppercase tracking-tighter">{t('market.unit_price')}</div>
      ...
      <div className="bento-card p-4 bg-accent-orange/5 border-accent-orange/20 text-center">
        <div className="flex items-center justify-center gap-2 text-accent-orange mb-2">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest">{t('market.protocol')}</span>
        </div>
        <p className="text-[10px] text-zinc-500 leading-relaxed font-mono">
          {t('market.protocol_desc')}
        </p>
      </div>
    </div>
  );
